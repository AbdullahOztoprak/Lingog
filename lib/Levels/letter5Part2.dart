...
import 'package:lingogame/components/grid5.dart';
...
class Letter5Part2 extends StatefulWidget {
  ...
  const Letter5Part2({super.key, required this.wordLength});

  @override
  State<Letter5Part2> createState() => _Letter5Part2State();
}

class _Letter5Part2State extends State<Letter5Part2> {
  late String _word;
  ...
  void initState() {
    ...
    final wordList =
        words5.where((word) => word.length == widget.wordLength).toList();

    if (wordList.isNotEmpty) {
      final r = Random().nextInt(wordList.length);
      _word = wordList[r];
    } else {
      _word = "ASKER"; // Eğer kelime yoksa varsayılan bir değer koy
    }
      ...
      child: Row(
        ...
        children: List.generate(correctWord.length, (index) {
          return Padding(
            ...
            child: Container(
              width: size.width * 0.135, // 📌 Dinamik genişlik
              height: size.width * 0.135, // 📌 Dinamik yükseklik
