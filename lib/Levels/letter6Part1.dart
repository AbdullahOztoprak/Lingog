...
import 'package:lingogame/Levels/letter6Part2.dart';
import 'package:lingogame/components/grid6.dart';
...
class Letter6Part1 extends StatefulWidget {
  final int wordLength; // ✅ Kelime uzunluğu

  const Letter6Part1({super.key, required this.wordLength});

  @override
  State<Letter6Part1> createState() => _Letter6Part1State();
}

class _Letter6Part1State extends State<Letter6Part1> {
  late String _word;
  ...
  void initState() {
    ...
    final wordList =
        words6.where((word) => word.length == widget.wordLength).toList();

    if (wordList.isNotEmpty) {
      final r = Random().nextInt(wordList.length);
      _word = wordList[r];
    } else {
      _word = "AKRABA"; // Eğer kelime yoksa varsayılan bir değer koy
    }
      ...
      child: Row(
        ...
        children: List.generate(correctWord.length, (index) {
          return Padding(
            ...
            child: Container(
              width: size.width * 0.105, // 📌 Dinamik genişlik
              height: size.width * 0.105, // 📌 Dinamik yükseklik
              ...
              child: Text(
                ...
                style: TextStyle(
                  color: Colors.white,
                  fontSize: size.width * 0.06, // 📌 Dinamik font
                  fontWeight: FontWeight.bold,
                ),
              ),
            ),
